      ...
      <div className="px-1.5 sm:px-8">
        <div className="flex items-center justify-between h-16 sm:h-20 gap-1 sm:gap-2">
          ...
          <div className="flex items-center gap-2 sm:gap-3.5 flex-shrink-0">
            ...
            <Link to="/" className="flex items-center gap-2 sm:gap-3 group select-none">
              ...
              <div className="hidden sm:flex flex-col leading-none">
                ...
                <span className="text-[10px] sm:text-xs tracking-widest uppercase font-bold text-emerald-700 dark:text-emerald-400 hidden sm:block mt-0.5">
                  {t('appSubtitle')}
                </span>
              </div>
            </Link>
          ...
          <div className="hidden lg:flex items-center gap-2.5 dark:bg-white/5 bg-slate-100 dark:border-white/10 border-slate-200 border px-4 py-1.5 rounded-full shadow-inner">
            ...
            <span className="text-xs sm:text-sm font-extrabold text-amber-600 dark:text-amber-400 font-mono">SYS-042</span>
          </div>

          {/* ── Right: Role Switcher + Language + Theme + TTS Voice + Alerts + Avatar ── */}
          <div className="flex items-center gap-1.5 sm:gap-3 flex-shrink-0">
                ...
                {['Resident', 'Technician', 'Admin'].map((role) => (
                  <button
                    ...
                    className={`px-1.5 sm:px-2.5 py-1 rounded-lg font-bold transition ${
                      userRole === role
                        ? 'bg-emerald-800 text-white shadow-sm ring-1 ring-amber-400/40'
            ...
            {/* Universal Voice Text-to-Speech Button (temporarily commented out)
            <VoiceSpeakerButton
              text={`${t('appName')} System Telemetry. Status is active. Sensor Node SYS-042 reporting normal drainage level and optimal underground capacity.`}
              label="Listen"
              showLabel={false}
              size="sm"
              variant="pill"
              id="navbar-global-tts"
              className="p-2 sm:p-2.5 rounded-xl border dark:border-white/10 border-slate-200 shadow-sm"
            />
            */}

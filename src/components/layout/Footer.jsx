    ...
    <footer className="bg-white dark:bg-[#0F172A] border-t border-slate-200/80 dark:border-white/10 py-8 px-4 sm:px-6 lg:px-8 mt-auto transition-colors duration-200">
      <div className="max-w-7xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4">
        <div className="flex items-center gap-2">
          <Logo size="xs" />
          <span className="font-extrabold text-sm text-slate-900 dark:text-white font-display">SUOW<span className="text-amber-500">MRS</span></span>
        ...
        <div className="flex flex-wrap items-center justify-center gap-6 text-xs text-slate-600 dark:text-slate-300 font-medium">
          <Link to="/" className="hover:text-emerald-700 dark:hover:text-emerald-400 transition">{t('home')}</Link>
          <Link to="/dashboard" className="hover:text-emerald-700 dark:hover:text-emerald-400 transition">{t('residentPortal')}</Link>
          <Link to="/admin/dashboard" className="hover:text-emerald-700 dark:hover:text-emerald-400 transition">{t('adminDashboard')}</Link>
          <Link to="/login" className="hover:text-emerald-700 dark:hover:text-emerald-400 transition">{t('signIn')}</Link>
        </div>

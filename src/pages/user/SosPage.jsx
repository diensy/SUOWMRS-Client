...
const EMERGENCY_TYPES = [
  // label/desc are English fallbacks (also sent to the backend); labelKey/descKey drive the UI text
  { id: 'flood',     icon: AlertOctagon, label: 'Active Flooding',       labelKey: 'sos_type_flood',    descKey: 'sos_type_flood_desc',    color: '#EF4444', desc: 'Water entering homes or streets' },
  { id: 'blockage',  icon: Siren,        label: 'Drain Blockage',        labelKey: 'sos_type_blockage', descKey: 'sos_type_blockage_desc', color: '#F59E0B', desc: 'Clogged drain causing overflow'  },
  { id: 'overflow',  icon: Radio,        label: 'Sewer Overflow',        labelKey: 'sos_type_overflow', descKey: 'sos_type_overflow_desc', color: '#8B5CF6', desc: 'Sewage overflow in public area'  },
  { id: 'hazard',    icon: Shield,       label: 'Safety Hazard',         labelKey: 'sos_type_hazard',   descKey: 'sos_type_hazard_desc',   color: '#0EA5E9', desc: 'Structural or road damage'       },
  { id: 'collapse',  icon: Zap,          label: 'Infrastructure Failure',labelKey: 'sos_type_collapse', descKey: 'sos_type_collapse_desc', color: '#F97316', desc: 'Pipe burst or channel collapse'  },
];

const EMERGENCY_CONTACTS = [
  { labelKey: 'sos_contact_control_room', number: '0674-2536820', icon: Phone,   available: true  },
  { labelKey: 'sos_contact_flood_helpline', number: '1070',       icon: Siren,   available: true  },
  { labelKey: 'sos_contact_ndrf',           number: '011-24363260', icon: Shield, available: true  },
  { labelKey: 'sos_contact_police',         number: '100',        icon: User,    available: true  },
];
  ...
  useEffect(() => {
    ...
    if (countdown === 0) {
      ...
      const newEntry = {
        ...
        typeKey: selectedType.labelKey,
        icon: selectedType.id,
        color: selectedType.color,
        location: location || t('sos_locationNotProvided'),
        description: description || t('sos_noDetails'),
      ...
      <div className="pb-4 border-b border-rose-200 dark:border-rose-500/20 flex items-start justify-between gap-3 flex-wrap">
        ...
        {dispatched.length > 0 && (
          ...
          >
            ...
            {t('sos_history')} ({dispatched.length})
          </button>
        )}
      </div>
      ...
      <AnimatePresence mode="wait">
                ...
                {EMERGENCY_TYPES.map(type => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div>
                        <p className="text-sm font-bold text-slate-900 dark:text-white">{t(type.labelKey)}</p>
                        <p className="text-[11px] text-slate-500 dark:text-slate-400">{t(type.descKey)}</p>
                      </div>
        ...
        {step === 'confirm' && (
          <motion.div key="confirm" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }}
            ...
            <div className="text-center">
              <p className="text-xs font-black tracking-widest text-slate-500 dark:text-slate-400 uppercase">{t('sos_dispatchingIn')}</p>
              <p className="text-7xl font-black text-rose-500 dark:text-rose-400 my-2 font-mono">{countdown}</p>
              <p className="text-sm text-slate-600 dark:text-slate-400 font-medium">
                {t('sos_emergency')}: <strong className="text-slate-900 dark:text-white">{selectedType ? t(selectedType.labelKey) : ''}</strong>
              </p>
              {location && (
        ...
        {step === 'sent' && (
          <motion.div key="sent" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }}
            ...
            <div className="text-center">
              <h2 className="text-xl font-black text-slate-900 dark:text-white">{t('sos_alertDispatched')}</h2>
              <p className="text-sm text-slate-600 dark:text-slate-400 mt-1">
                {t('sos_alertDispatchedDesc')}
              </p>
              <div className="mt-3 inline-flex items-center gap-2 bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-500/20 rounded-xl px-3 py-1.5">
                ...
                <span className="text-xs font-bold">{t('sos_emergencyId')}: SOS-{Date.now().toString().slice(-6)}</span>
              </div>
            </div>
            {/* Emergency contacts */}
            <div className="w-full bg-white dark:bg-slate-900/60 border border-slate-200 dark:border-white/10 rounded-2xl p-4">
              <p className="text-xs font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-3">{t('sos_emergencyContacts')}</p>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex-1">
                        <p className="text-xs font-bold text-slate-900 dark:text-white">{t(c.labelKey)}</p>
        ...
        {step === 'history' && (
          <motion.div key="history" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="text-sm font-bold text-slate-900 dark:text-white">{t('sos_yourHistory')}</h3>
              <button onClick={() => setStep('select')} className="text-xs text-sky-600 dark:text-sky-400 hover:underline font-bold">
                ← {t('back')}
              </button>
            </div>
            {dispatched.length === 0 ? (
              <div className="text-center py-12 text-slate-400 dark:text-slate-500 text-sm">{t('sos_noneYet')}</div>
            ) : (
              dispatched.map(entry => (
                <div key={entry.id} className="flex items-start gap-3 p-4 rounded-xl bg-white dark:bg-slate-900/60 border border-slate-200 dark:border-white/10 shadow-sm">
                  ...
                  <div className="flex-1">
                    <p className="text-sm font-bold text-slate-900 dark:text-white">{entry.typeKey ? t(entry.typeKey) : entry.type}</p>
